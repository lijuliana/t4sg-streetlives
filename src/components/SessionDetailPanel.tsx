...
import { Plus, MessageSquare, Circle, UserPlus, ArrowRight, CheckCircle, RotateCcw, X } from "lucide-react";
...
export default function SessionDetailPanel({ sessionId, onClose, onOpenChat }: Props) {
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
        ...
        {!isClosed && !isSupervisor && isMySession && (
          <button
            ...
            className="w-full bg-gray-900 text-white text-sm font-medium py-2.5 rounded-md hover:bg-gray-800 transition"
          >
            Close Session
          </button>
        )}
        ...
        {/* Wrap-up form */}
        {showCloseConfirm && (
          <div className="bg-white border border-gray-200 rounded-md px-4 py-4 space-y-4">
        ...
        {isClosed && session.logged && session.sessionLog && (
          <div>
            <h2 className="text-xs font-normal text-gray-500 uppercase tracking-wide mb-2">Outcome Log</h2>
            <div className="bg-white border border-gray-200 rounded-md px-4 py-3 space-y-3">
        ...
        <div>
          ...
          {session.referrals.length === 0 ? (
            <div className="bg-white border border-gray-200 rounded-md px-4 py-5 text-center">
        ...
        {!isSupervisor && isClosed && session.logged && (
          <div>
            ...
            {session.reviewStatus === "submitted" && (
              <div className="bg-white border border-gray-200 rounded-md px-4 py-3 text-center">
        ...
        {isSupervisor && (session.reviewStatus === "approved" || session.reviewStatus === "returned") && (
          <div>
            <h2 className="text-xs font-normal text-gray-500 uppercase tracking-wide mb-2">Supervisor Review</h2>
            <div className="bg-white border border-gray-200 rounded-md px-4 py-4 space-y-2">
              {session.reviewStatus === "approved" ? (
